from django.contrib.auth.decorators import login_required
from django.contrib.auth.mixins import LoginRequiredMixin, \
    PermissionRequiredMixin
from django.core.paginator import Paginator
from django.http import HttpResponseNotFound
from django.shortcuts import render, get_object_or_404
from django.urls import reverse_lazy
from django.views.generic import ListView, DetailView, CreateView, UpdateView

from women.forms import AddPostForm
from women.models import Women, TagPost
from women.utils import DataMixin


class WomenHome(DataMixin, ListView):
    """Отображение страниц со статьями"""

    template_name = 'women/index.html'
    context_object_name = 'posts'

    def get_queryset(self):
        """Список всех опубликованных статей"""
        return Women.published.all().select_related('cat')

    def get_context_data(self, *, object_list=None, **kwargs):
        """Передача данных полей в шаблон"""
        context = super().get_context_data(**kwargs)
        return self.get_mixin_context(
            context, title="Главная страница", cat_selected=0
        )


class AddPage(PermissionRequiredMixin, LoginRequiredMixin,
              DataMixin, CreateView):
    """Добавление статьи"""

    form_class = AddPostForm
    template_name = 'women/addpage.html'
    success_url = reverse_lazy('home')
    title_page = "Добавление статьи"
    permission_required = 'women.add_women'

    def form_valid(self, form):
        """Возвращает обратно на страницу, если данные успешно отправлены"""
        w = form.save(commit=False)
        w.author = self.request.user
        return super().form_valid(form)


class UpdatePage(PermissionRequiredMixin, DataMixin, UpdateView):
    """Обновление статьи"""

    model = Women
    fields = ['title', 'content', 'photo', 'is_published', 'cat']
    template_name = 'women/addpage.html'
    success_url = reverse_lazy('home')
    title_page = "Редактирование статьи"
    permission_required = 'women.change_women'


class ShowPost(DataMixin, DetailView):
    """Отображение конкретной статьи"""

    model = Women
    template_name = 'women/post.html'
    slug_url_kwarg = 'post_slug'
    context_object_name = 'post'
    allow_empty = False

    def get_context_data(self, **kwargs):
        """Передача данных полей в шаблон"""
        context = super().get_context_data(**kwargs)
        return self.get_mixin_context(context, title=context['post'])

    def get_object(self, queryset=None):
        """Отображение в зависимости от статуса статьи"""
        return get_object_or_404(
            Women.published, slug=self.kwargs[self.slug_url_kwarg])


class WomenCategory(DataMixin, ListView):
    """Отображение страниц со статьями с фильтром по категории"""

    template_name = 'women/index.html'
    context_object_name = 'posts'
    allow_empty = False

    def get_context_data(self, *, object_list=None, **kwargs):
        """Передача данных полей в шаблон"""

        context = super().get_context_data(**kwargs)
        cat = context['posts'][0].cat

        return self.get_mixin_context(
            context, title=f"Категория - {cat.name}", cat_selected=cat.id
        )

    def get_queryset(self):
        """Список опубликованных статей с учётом категории"""
        return Women.published.filter(
            cat__slug=self.kwargs['cat_slug']).select_related('cat')


class TagPostList(DataMixin, ListView):
    """Отображение страниц со статьями с фильтром по тегам"""

    template_name = 'women/index.html'
    context_object_name = 'posts'
    allow_empty = False

    def get_context_data(self, *, object_list=None, **kwargs):
        """Передача данных полей в шаблон"""

        context = super().get_context_data(**kwargs)
        tag = TagPost.objects.get(slug=self.kwargs['tag_slug'])
        return self.get_mixin_context(context, title=f"Тег: {tag.tag}")

    def get_queryset(self):
        """Список опубликованных статей с учётом тегов"""
        return Women.published.filter(
            tags__slug=self.kwargs['tag_slug']).select_related('cat')


@login_required
def about(request):
    """Отображение страниц с именами женщин из статей"""

    contact_list = Women.published.all()
    paginator = Paginator(contact_list, 3)

    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)

    return render(request, 'women/about.html',
                  {'title': "О сайте", 'page_obj': page_obj})


def page_not_found(request, exception):
    return HttpResponseNotFound("<h1>Страница не найдена</h1>")
